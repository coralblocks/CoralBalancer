...
CoralBalancer is a deterministic, garbage-free, fast _key-to-node_ balancer for Java. It uses [Rendezvous Hashing](https://en.wikipedia.org/wiki/Rendezvous_hashing) to balance any key (of any type) uniformly by deterministically choosing a node from a set of nodes. It is meant to be used in deterministic, single-topic, single-threaded, event-stream architectures where every node receives all messages. Each node builds the same balancer state, then uses the `isForMe(key)` method to decide whether it should handle a message or not.
